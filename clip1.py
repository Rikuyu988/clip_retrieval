...
class CLIP(object):
    ...
    def detect_image(self, image, texts):
        ...
        #   CenterCrop
        image_data = letterbox_image(image, [self.config['input_resolution'], self.config['input_resolution']],
        ...
        with torch.no_grad():
            ...
            if self.cuda:
                images = images.cuda()
            #   将图像输入网络当中进行预测！
            logins_per_image, logins_per_text = self.net(images, texts)

            probs = logins_per_image.softmax(dim=-1).cpu().numpy()
